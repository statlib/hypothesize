...
from hypothesize.compare_groups_with_single_factor import lindepbt, rmmcppb
from hypothesize.utilities import (bptdpsi, con1way, con2way, covmtrim, lincon,
                                   lindep, linhat, pandas_to_arrays,
                                   remove_nans_based_on_design, rmanogsub,
                                   rmmcp, yuen)

# np.set_printoptions(linewidth=300)


def bwmcp(J, K, x, alpha=0.05, tr=0.2, nboot=599, seed=False):
    """
    ...
    x = remove_nans_based_on_design(
        x, design_values=[J, K], design_type="between_within"
    )
...
def bwamcp(J, K, x, tr=0.2, alpha=0.05, pool=False):
    """
...
def bwbmcp(J, K, x, tr=0.2, con=None, alpha=0.05, dif=True, pool=False, hoch=False):
    """
...
def bwimcp(J, K, x, tr=0.2, alpha=0.05):
    """
...
def spmcpa(J, K, x, est, *args, avg=False, alpha=0.05, nboot=None, seed=False):
    """
    All pairwise comparisons among levels of Factor A
...
def spmcpb(J, K, x, est, *args, dif=True, alpha=0.05, nboot=599, seed=False):
    """
    All pairwise comparisons among levels of Factor B
    ...
    results = {"output": test_res, "con": temp["con"], "num_sig": temp["num_sig"]}

    return results


def spmcpi(J, K, x, est, *args, alpha=0.05, nboot=None, SR=False, seed=False):
    """
    Multiple comparisons for interactions
    in a split-plot design.
    The analysis is done by taking difference scores
...
def wwmcppb(
    ...
    *args,
    ...
    x = pandas_to_arrays(x)
    x = remove_nans_based_on_design(
        x, design_values=[J, K], design_type="within_within"
    )
    x_mat = np.r_[x].T
...
def wwmcpbt(J, K, x, tr=0.2, alpha=0.05, nboot=599, seed=False):
    """
...
def bwmcppb(J, K, x, est, *args, alpha=0.05, nboot=500, bhop=True, seed=True):
    """
    ...
    print(
        "ask wilcox if this is onlly for trimmed means " "depite the optional est arg",
    ...
    A = bwmcppb_sub(
        J, K, x, est, *args, con=conA, alpha=alpha, nboot=nboot, bhop=bhop, seed=seed
    )

    B = bwmcppb_sub(
        J, K, x, est, *args, con=conB, alpha=alpha, nboot=nboot, bhop=bhop, seed=seed
    )

    AB = bwmcppb_sub(
        J, K, x, est, *args, con=conAB, alpha=alpha, nboot=nboot, bhop=bhop, seed=seed
    )
    ...
    results = {
        "factor_A": pd.DataFrame(A),
        "factor_B": pd.DataFrame(B),
        "factor_AB": pd.DataFrame(AB),
    }
...
def bwmcppb_sub(
    J, K, x, est, *args, con=None, alpha=0.05, nboot=500, bhop=True, seed=True
):
    """
