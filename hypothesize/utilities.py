...
def trimci(x, tr=0.2, alpha=0.05, null_value=0):
    """
...
def lincon(x, con=None, tr=0.2, alpha=0.05, seed=False):
    """
    ...
    if tr >= 0.5:
        raise Exception(
            "Use the function medpb to compare medians (it may not be implemented in this package"
        )

    flag = True if alpha == 0.05 or alpha == 0.01 else False
...
def smmcrit(nuhat, C):
    ...
    if C == 1:
        smmcrit_result = t.ppf(0.975, nuhat)
    ...
    if C >= 2:
        ...
        if temp[find[0]] != 0:
            if nuhat > nu[find[0]]:
                smmcrit_result = m1[find[0], C] - (1 / nu[find[0]] - 1 / nuhat) * (
                    m1[find[0], C] - m1[find[0] + 1, C]
                ) / (1 / nu[find[0]] - 1 / nu[find[0] + 1])

        if nuhat < nu[find[0]]:
            smmcrit_result = m1[find[0] - 1, C] - (1 / nu[find[0] - 1] - 1 / nuhat) * (
                m1[find[0] - 1, C] - m1[find[0], C]
...
def smmcrit01(nuhat, C):
    ...
    if C == 1:
        smmcrit01_result = t.ppf(0.995, nuhat)
    ...
    if C >= 2:
        ...
        if temp[find[0]] != 0:
            if nuhat > nu[find[0]]:
                smmcrit01_result = m1[find[0], C] - (1 / nu[find[0]] - 1 / nuhat) * (
                    m1[find[0], C] - m1[find[0] + 1, C]
                ) / (1 / nu[find[0]] - 1 / nu[find[0] + 1])

        if nuhat < nu[find[0]]:
            smmcrit01_result = m1[find[0] - 1, C] - (
                1 / nu[find[0] - 1] - 1 / nuhat
            ) * (m1[find[0] - 1, C] - m1[find[0], C]) / (
                1 / nu[find[0] - 1] - 1 / nu[find[0]]
            )
...
def covmtrim(x, tr):
    ...
    for j in range(1, p):
        ...
        for k in range(j):
            covest[j, k] = (n - 1) * wincor(x[j], x[k], tr)["wcov"] / (h * (h - 1))
...
def yuen(x, y, tr=0.2, alpha=0.05):
    ...
    if tr == 0.5:
        raise Exception(
            "Using tr=.5 is not allowed; use a method designed for medians "
    ...
    results = {
        "n1": len(x),
        "n2": len(y),
        "est_1": trim_mean(x, tr),
        "est_2": trim_mean(y, tr),
        "ci": [low, up],
        "p_value": yuen_results,
        "dif": dif,
        "se": np.sqrt(q1 + q2),
        "test_stat": test,
        "crit": crit,
...
def yuend(x, y, tr=0.2, alpha=0.05):
    ...
    keys = ["ci", "p_value", "est1", "est2", "dif", "se", "teststat", "n", "df"]
...
def create_example_data(
    design_values, missing_data_proportion=0, save_array_path=None, seed=False
):
    """
    ...
    if missing_data_proportion:
        x.ravel()[
            np.random.choice(
                x.size, int(round(missing_data_proportion * x.size)), replace=False
            )
        ] = np.nan
...
def rmmcp(x, con=None, tr=0.2, alpha=0.05, dif=True, hoch=True):
    """
    ...
    if alpha == 0.05:
        ...
        if ncon > 10:
            avec = 0.05 / np.arange(ncon, 11 + 1)[::-1]
    ...
    if alpha == 0.01:
        ...
        if ncon > 10:
            avec = 0.01 / np.arange(ncon, 11 + 1)[::-1]
            dvec = np.concatenate([dvec, avec])
    ...
    if con is None:  # sum(con**2)==0:
            ...
            for k in range(J):
                if j < k:
                    ...
                    if not dif:
                        ...
                        temp1 = np.append(
                            temp1, 2 * (1 - t.cdf(abs(test[jcom, 2]), df))
                        )
        ...
        if con.shape[0] != x.shape[1]:
            raise Exception(
                "The number of groups does not match the number "
    ...
    return {"n": nval, "test": test, "psihat": psihat, "con": con, "num_sig": num_sig}


def trimcibt(x, tr=0.2, alpha=0.05, nboot=None, seed=False):
    """
    Compute a 1-alpha confidence interval for the trimmed mean
