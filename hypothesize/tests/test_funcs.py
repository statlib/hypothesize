...
from hypothesize.utilities import con1way, create_example_data, trim_mean
    ...
    if type(expected_results) is list:
        ...
        for k in expected_results:
            if type(expected_results[k]) is np.ndarray:
                ...
                truth = (
                    True
                    if not np.testing.assert_allclose(
                        expected_results[k], actual_results[k]
                    )
                    else False
                )
                ...
                truth = (
                    True
                    if not assert_frame_equal(
                        expected_results[k], actual_results[k], check_less_precise=True
                    )
                    else False
                )
                ...
                nested_truth = check_dict_items_equality(
                    expected_results[k], actual_results[k]
                )
                actual_truth.append(nested_truth)
                ...
                if expected_results[k] is None and actual_results[k] is None:
                    ...
                    truth = (
                        True
                        if not np.testing.assert_almost_equal(
                            expected_results[k], actual_results[k]
                        )
                        else False
                    )
