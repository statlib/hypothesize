...
from hypothesize.utilities import (bptdpsi, con1way, con2way, lincon,
                                   pandas_to_arrays,
                                   remove_nans_based_on_design, rmmcp,
                                   trimcibt, trimparts, trimpartt, trimse,
                                   yuend)


def yuenbt(x, y, tr=0.2, alpha=0.05, nboot=599, seed=False):
    """
    ...
    test_stat = (trim_mean(x, tr) - trim_mean(y, tr)) / np.sqrt(
        trimse(x, tr=tr) ** 2 + trimse(y, tr=tr) ** 2
    )

    datax = np.random.choice(xcen, size=(nboot, len(x)))
    datay = np.random.choice(ycen, size=(nboot, len(y)))

    top = trim_mean(datax, 0.2, axis=1) - trim_mean(datay, 0.2, axis=1)
    ...
    tval = abs(tval)
    tval = sorted(tval)
    icrit = int(np.floor((1 - alpha) * nboot + 0.5))
...
def linconb(x, con, tr=0.2, alpha=0.05, nboot=599, seed=False):
    """
    ...
    for d in range(con.shape[1]):
        ...
        psihat[d, 0] = d
        testit = lincon(
            x, np.array([con[:, d]]).T, tr, alpha
        )  # column slice of contrast matrix
        ...
        psihat[d, 2] = (
            testit["psihat"]["psihat"][0] - testb[ic] * testit["test"]["se"][0]
        )
        psihat[d, 3] = (
            testit["psihat"]["psihat"][0] + testb[ic] * testit["test"]["se"][0]
        )
...
def rmmcppb(
    ...
    *args,
    ...
    if dif:
        print(
            "analysis is being done on difference scores",
            "each confidence interval has probability coverage of 1-alpha.",
        )

        temp = rmmcppbd(x, est, *args, alpha=alpha, con=con, nboot=nboot, hoch=True)
        ...
        if not BA:
            print(
                "If and when MOM and/or onestep estimators are implemeted, "
        ...
        for ic in range(connum):
            ...
            ptemp = (
                np.sum(psihat[ic, :] > 0) + 0.5 * np.sum(psihat[ic, :] == 0)
            ) / nboot
...
def rmmcppbd(x, est, *args, alpha=0.05, con=None, nboot=None, hoch=True, seed=False):
    """
    Use a percentile bootstrap method to compare dependent groups
    ...
    for ic in range(connum):
        test[ic] = (sum(psihat[ic, :] > 0) + 0.5 * sum(psihat[ic, :] == 0)) / nboot
...
def lindepbt(x, tr=0.2, con=None, alpha=0.05, nboot=599, dif=True, seed=False):
    """
    ...
    if type(x) is pd.DataFrame:
        x = pandas_to_arrays(x)
        x = remove_nans_based_on_design(
            x, design_values=len(x), design_type="dependent_groups"
        )
        x = np.r_[x].T
        called_directly = True
    ...
    for d in range(ncon):
        ...
        if not dif:
            ...
            bvec = [lindep_sub(data_row, xcen, con[:, d], tr=tr) for data_row in data]
...
def lindep_sub(data, x, con=None, tr=0.2):
...
def pb2gen(x, y, est, *args, alpha=0.05, nboot=2000, seed=False):
    """
...
def bootdpci(x, est, *args, nboot=None, alpha=0.05, dif=True, BA=False, SR=False):
    """
    ...
    if SR and est not in ("onestep", "mom"):
        SR = False
        print(
            "setting SR to False. SR=True should apparently "
    ...
    results = rmmcppb(x, est, *args, nboot=nboot, alpha=alpha, SR=SR, dif=dif, BA=BA)
...
def ydbt(x, y, tr=0.2, alpha=0.05, nboot=599, side=True, seed=False):
    """
...
def tmcppb(x, est, *args, con=None, bhop=False, alpha=0.05, nboot=None, seed=False):
    """
    ...
    for d in range(ncon):
        ...
        test[d] = np.sum(bcon[d, :] > 0) / nboot + 0.5 * tv
        if test[d] > 0.5:
            test[d] = 1 - test[d]
...
def l2drmci(
    x, y, est, *args, pairwise_drop_na=True, alpha=0.05, nboot=2000, seed=False
):
    """
    ...
    for i in range(nboot):
        bvec[i] = loc2dif(
            x[data[i, :], 0], x[data[i, :], 1], est, *args, drop_na=pairwise_drop_na
        )
